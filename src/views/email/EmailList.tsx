...
const EmailList: React.FC<EmailListProps> = ({
  ...
  const fetchEmails = async () => {
    try {
      ...
      const response = await Api.xPotatoApi.getEmailByPage({ currentPage, pageSize });
      console.log(
        '%c [ response ]-74',
  ...
  const renderEmailItem = (email: typeEmail) => (
    <ListItem
      key={email.emailId}
      className={`border-1 group relative cursor-pointer border-b-2 border-b-blue-gray-200 bg-gray-100 text-blue-gray-900 transition-colors duration-200 hover:bg-gray-300 dark:bg-blue-gray-900 dark:text-gray-200 dark:hover:bg-blue-gray-800`}
  ...
  return (
    <>
      <Card className="h-[calc(100%-80px)] w-full justify-between overflow-y-scroll bg-gray-100 dark:bg-blue-gray-900">
        ...
        {state.status === 'error' && (
          <Alert
            color="red"
            className="m-4 bg-red-50 text-red-500 dark:bg-red-900/30 dark:text-red-400"
            action={
              <IconButton
                size="sm"
        ...
        {state.status === 'success' && (
          <>
            {state.data.length === 0 ? (
              ...
              <>
                <List className="p-0">{state.data.map(renderEmailItem)}</List>
                {renderPagination()}
              </>
            )}
          </>
