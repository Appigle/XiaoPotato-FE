...
export function NavbarWithSearch() {
  ...
  const navList = (
    <ul className="mb-4 mt-2 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      <Typography
        ...
        variant="small"
        color="blue-gray"
        className="hidden items-center gap-x-2 p-1 font-medium"
      >
        <svg
          width="16"
          height="15"
      ...
      >
        <a href="#" className="flex items-center" title="profile">
          <svg
            width="32"
            height="34"
            viewBox="0 0 16 17"
      ...
      <Typography
        ...
        variant="small"
        color="blue-gray"
        className="flex hidden items-center gap-x-2 p-1 font-medium"
      >
        <svg
          width="14"
          height="15"
  ...
  const [artType, setArtType] = React.useState('Painting');
  const [curArType, setCurArtType] = React.useState<ART_TYPE_ITEM>({
    emoji: '🎨',
    name: 'Painting',
    desc: '🖼️ (framed picture) - A visual art form that uses pigments to create images on a surface.',
  });
  console.log('%c [ curArType ]-156', 'font-size:13px; background:pink; color:#bf2c9f;', curArType);

  React.useEffect(() => {
    window.addEventListener('resize', () => window.innerWidth >= 960 && setOpenNav(false));
  }, []);
  React.useEffect(() => {
    setCurArtType(ART_TYPES.find((f) => f.name === artType) as ART_TYPE_ITEM);
  }, [artType]);

  return (
    <Navbar className="w-full !max-w-full !rounded-none bg-potato-white from-blue-gray-900 to-blue-gray-800 px-4 py-2">
            ...
            <Menu open={openMenu} handler={setOpenMenu}>
              <MenuHandler>
                <Button
                  ...
                  variant="outlined"
                  size="sm"
                  className="flex h-10 cursor-pointer flex-nowrap items-center gap-2 text-ellipsis rounded-r-none border border-r-0 border-blue-gray-200 bg-blue-gray-500/10 pl-3"
                >
                  <span className="whitespace-nowrap">
                    {curArType.emoji} {artType}
                  </span>
                  <ChevronDownIcon
                    strokeWidth={2.5}
              ...
              <MenuList className="hidden max-h-72 w-20 lg:block">
                {ART_TYPES.map(({ name, emoji }) => (
                  <MenuItem
                    key={name}
                    className="hover:outline-none"
                    onClick={() => setArtType(name)}
                  >
                    <span
                      title={name}
